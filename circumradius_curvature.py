...
manim -pql circumradius_curvature.py CircumradiusCurvatureCollapsingDiscreteCurve
...
class CircumradiusCurvatureCollapsingDiscreteCurve(Scene):
    def construct(self):
        ...
        t = 0
        curr_curvature = Dot(radius=0.01, color=GREEN).move_to(list(ddg.Point(t, curve.total_curvature_circumradius()/5)+ddg.Point(*list(axes_curvature.get_origin())[:-1])) + [0])
        ...
        for _ in range(450):
            ...
            vectors = [Vector(list(curve.nth_unit_vector_along_circumcenter(i) * curve.nth_circumradius_curvature(i)), color=DARK_BLUE) for i in range(len(curve))]

            for i in range(len(curve)):
                vi = vectors[i]
                vi.shift(list(curve.points[i]) + [0])

            curve = curve.timestep_from_circumradius_curvature(time_step=0.008)
            t += 0.008
            ...
            new_vectors = [Vector(list(curve.nth_unit_vector_along_circumcenter(i) * curve.nth_circumradius_curvature(i)), color=DARK_BLUE) for i in range(len(curve))]
            ...
            self.play(*[Transform(dots[i], new_dots[i]) for i in range(len(points))],
                      ...
                      *[Transform(vectors[i], new_vectors[i]) for i in range(len(vectors))],
                      ...
                      ApplyMethod(curr_curvature.move_to, list(ddg.Point(t/2, curve.total_curvature_circumradius()/5)+ddg.Point(*list(axes_curvature.get_origin())[:-1])) + [0]), run_time=0.1)
